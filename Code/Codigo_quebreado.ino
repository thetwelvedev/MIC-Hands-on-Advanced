...
#include <Firebase_ESP_Client.h> // nome atualizado da lib

// Configurações do Firebase
#define API_KEY "YUqCN9J30XHtgZIQAb4PdcHMck1rV08HlGDC8LvW"
#define DATABASE_URL "https://monitoramento-bpm-e-temp-default-rtdb.firebaseio.com/"  // sem aspas no final!
...
// Deixe o auth.user e auth.password vazios se estiver no modo teste
auth.user.email = "";
auth.user.password = "";
...
// Enviar dados para o Firebase (versão correta para Firebase_ESP_Client v4+)
Firebase.RTDB.setFloatAsync(&firebaseData, "/paciente/temperatura", calibratedTemp);
Firebase.RTDB.setBoolAsync(&firebaseData, "/paciente/alerta_febre", overTemp);
Firebase.RTDB.setIntAsync(&firebaseData, "/paciente/bpm", (int)beatsPerMinute);
Firebase.RTDB.setIntAsync(&firebaseData, "/paciente/media_bpm", beatAvg);
